...
public class LoginServlet1 extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// 入力されたIDをもとにソルトを取得する。
		String salt = Account1DAO.getSalt(mail);
		
		// 取得したソルトがnullの場合は対象のユーザがいないので、Errorでログイン画面に戻す
		if(salt == null) {
			String view = "WEB-INF/view/sample-login1.jsp?error=1";
		...
		Account4 account = Account1DAO.login1(mail, hashedPw);
		...
		if(account == null) {
			...
			String view = "WEB-INF/view/sample-menu1.jsp";
			RequestDispatcher dispatcher = request.getRequestDispatcher(view);
			dispatcher.forward(request, response);
		}
	}
